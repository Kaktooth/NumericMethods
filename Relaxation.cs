...
namespace NumberMethods
{
    public partial class Relaxation : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            () =>
            {
                Equation.RelaxIteration(0, 0, a, b, x, E, values, new bool[3]);
            });
            thread.Start();
            thread.Join();
            ...
            for (int j = 1; j < values.Count; j++)
            {
                ...
                data.row["i"] = j;
                data.row["delta 1"] = values[j][0].Item1;
                data.row["delta 2"] = values[j][0].Item2;
                data.row["delta 3"] = values[j][0].Item3;
                data.row["x1"] = values[j][0].Item4;
                data.row["x2"] = values[j][0].Item5;
                data.row["x3"] = values[j][0].Item6;
